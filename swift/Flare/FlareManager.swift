...
public protocol FlareManagerDelegate: class {
...
public class FlareManager: APIManager {
    ...
    public weak var delegate: FlareManagerDelegate?
    ...
    public init(host: String, port: Int) {
        ...
        self.server = "http://\(host):\(port)" // support https and subpaths
    }
    ...
    public func getMacAddress(host: String, port: Int, handler: @escaping (String) -> Void) {
    ...
    func finishRequest(handler: @escaping () -> Void) {
        requests -= 1
        if requests == 0 {
            handler()
        }
    ...
    public func loadEnvironment(environment: Environment, handler: @escaping () -> Void) {
        self.startRequest()
        self.listZones(environmentId: environment.id) { jsonArray -> Void in
            for json in jsonArray {
                ...
                self.startRequest()
                self.listThings(environmentId: environment.id, zoneId: zone.id) { jsonArray -> Void in
                    for json in jsonArray {
    ...
    public func loadEnvironments(handler: @escaping ([Environment]) -> Void) {
    ...
    func finishEnvironmentsRequest(handler: @escaping ([Environment]) -> Void, environments: [Environment]) {
        environmentsRequests -= 1
    ...
    public func loadEnvironments(params: JSONDictionary?, loadDevices: Bool, handler: @escaping ([Environment]) -> Void) {
        var environments = [Environment]()

        self.startEnvironmentsRequests()
        self.listEnvironments(params: params) { jsonArray -> Void in
            for json in jsonArray {
                ...
                if loadDevices {
                    self.startEnvironmentsRequests()
                    self.listDevices(environmentId: environment.id) { jsonArray -> Void in
                        for json in jsonArray {
    ...
    public func getCurrentZone(environment: Environment, device: Device, handler: @escaping (Zone) -> Void) {
    ...
    public func getNearestThing(environment: Environment, device: Device, handler: @escaping (Thing) -> Void) {
    ...
    public func modifyFlare(flare: Flare, handler: @escaping (JSONDictionary) -> (JSONDictionary)) {
        getFlare(flare: flare) {json in
            ...
            self.updateFlare(flare: flare, json: modifiedJson) { _ in }
        }
    }

    // gets the up-to-date JSON for a Flare object from the server
    public func getFlare(flare: Flare, handler: @escaping (JSONDictionary) -> Void) {
    ...
    public func newFlare(flare: Flare?, json: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        var template = json
        if flare == nil {
    ...
    public func updateFlare(flare: Flare, json: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        if let environment = flare as? Environment {
    ...
    public func deleteFlare(flare: Flare, handler: @escaping (JSONDictionary) -> Void) {
    ...
    public func flareWithName(array: [Flare], name: String) -> Flare? {
        return array.first(where: { $0.name == name })
    }
    ...
    public func listEnvironments(handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments") { json in
    ...
    public func listEnvironments(params: JSONDictionary?, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments", params: params) { json in
            handler(json as? JSONArray ?? [])
        }
    }

    public func newEnvironment(environment: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments", params: nil, method: .POST, message: environment) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getEnvironment(environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func updateEnvironment(environmentId: String, environment: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)", params: nil, method: .PUT, message: environment) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func deleteEnvironment(environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)", params: nil, method: .DELETE, message: nil) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }
    ...
    public func listZones(environmentId: String, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones") { json in
            handler(json as? JSONArray ?? [])
        }
    }

    // return only zones in the environment containing the given point
    public func listZones(environmentId: String, point: Point3D, handler: @escaping (JSONArray) -> Void) {
    ...
    public func listZones(environmentId: String, params: JSONDictionary?, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones", params: params) { json in
            handler(json as? JSONArray ?? [])
        }
    }

    public func newZone(environmentId: String, zone: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones", params: nil, method: .POST, message: zone) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getZone(zoneId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func updateZone(zoneId: String, environmentId: String, zone: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)", params: nil, method: .PUT, message: zone) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func deleteZone(zoneId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)", params: nil, method: .DELETE, message: nil) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }
    ...
    public func listThings(environmentId: String, zoneId: String, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things") { json in
            handler(json as? JSONArray ?? [])
        }
    }
    ...
    public func listThings(environmentId: String, zoneId: String, params: JSONDictionary?, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things", params: params) { json in
            handler(json as? JSONArray ?? [])
        }
    }

    public func newThing(environmentId: String, zoneId: String, thing: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things", params: nil, method: .POST, message: thing) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getThing(thingId: String, environmentId: String, zoneId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getThingData(thingId: String, environmentId: String, zoneId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)/data") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getThingDataValue(thingId: String, environmentId: String, zoneId: String, key: String, handler:@escaping (AnyObject) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)/data/\(key)", handler: handler)
    }

    public func getThingPosition(thingId: String, environmentId: String, zoneId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)/position") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func updateThing(thingId: String, environmentId: String, zoneId: String, thing: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)", params: nil, method: .PUT, message: thing) { json in handler(json as? JSONDictionary ?? [:])
        }
    }

    public func deleteThing(thingId: String, environmentId: String, zoneId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/zones/\(zoneId)/things/\(thingId)", params: nil, method: .DELETE, message: nil) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }
    ...
    public func listDevices(environmentId: String, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices") { json in
            handler(json as? JSONArray ?? [])
        }
    }
    ...
    // x, y, distance: filter things whose position is within distance from the given point
    // key, value: filter things whose data contains the given key/value pair
    public func listDevices(environmentId: String, params: JSONDictionary?, handler: @escaping (JSONArray) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices", params: params) { json in
            handler(json as? JSONArray ?? [])
        }
    }

    public func newDevice(environmentId: String, device: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices", params: nil, method: .POST, message: device) { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getDevice(deviceId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getDeviceData(deviceId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)/data") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func getDeviceDataValue(deviceId: String, environmentId: String, key: String, handler: @escaping (AnyObject) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)/data/\(key)", handler: handler)
    }

    public func getDevicePosition(deviceId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)/position") { json in
            handler(json as? JSONDictionary ?? [:])
        }
    }

    public func updateDevice(deviceId: String, environmentId: String, device: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)", params: nil, method: .PUT, message: device) {json in handler(json as? JSONDictionary ?? [:])
        }
    }

    public func deleteDevice(deviceId: String, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        sendRequest(uri: "environments/\(environmentId)/devices/\(deviceId)", params: nil, method: .DELETE, message: nil) {json in handler(json as? JSONDictionary ?? [:])
        }
    }
    ...
    public func newOrUpdateEnvironment(environment: JSONDictionary, handler: @escaping (JSONDictionary) -> Void) {
        if let id = environment["_id"] as? String, flareIndex[id] as? Environment != nil {
    ...
    public func newOrUpdateZone(zone: JSONDictionary, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        if let id = zone["_id"] as? String, flareIndex[id] as? Zone != nil {
    ...
    public func newOrUpdateThing(thing: JSONDictionary, environmentId: String, zoneId: String, handler: @escaping (JSONDictionary) -> Void) {
        if let id = thing["_id"] as? String, flareIndex[id] as? Thing != nil {
    ...
    public func newOrUpdateDevice(device: JSONDictionary, environmentId: String, handler: @escaping (JSONDictionary) -> Void) {
        if let id = device["_id"] as? String, flareIndex[id] as? Device != nil {
    ...
    public func getCurrentDevice(environmentId: String, template: JSONDictionary, handler: @escaping (Device?) -> Void) {
        self.savedDevice(environmentId: environmentId) { device -> Void in
            if device != nil {
                handler(device)
            } else {
                self.newDeviceObject(environmentId: environmentId, template: template) { device -> Void in
                    if device != nil {
                        handler(device)
                    }
                }
    ...
    public func savedDevice(environmentId: String, handler: @escaping (Device?) -> Void) {
        if let deviceId = UserDefaults.standard.string(forKey: "\(environmentId)-deviceId") {
            self.getDevice(deviceId: deviceId, environmentId: environmentId) { json -> Void in
                if json["_id"] as? String != nil {
    ...
    public func newDeviceObject(environmentId: String, template: JSONDictionary, handler: @escaping (Device?) -> Void) {
        newDevice(environmentId: environmentId, device: template) { json -> Void in
            let device = Device(json: json)
            self.addToIndex(flare: device)
    ...
    public func addHandlers() {
        ...
        socket.on("position") {messages, _ in
            if let message = messages[0] as? JSONDictionary,
                let flare = self.flareForMessage(message: message),
        ...
        socket.on("handleAction") {messages, _ in
            if let message = messages[0] as? JSONDictionary,
                let flare = self.flareForMessage(message: message),
